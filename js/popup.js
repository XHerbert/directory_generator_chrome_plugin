...
$('#update_bg_color').click(() => {
	...
	let excuteJs = '';
	excuteJs += 'let h = document.getElementsByClassName("page-title")[0].innerHTML;';
	excuteJs += 'alert(h);';
	executeScriptToCurrentTab(excuteJs);
});

// 修改字体大小
$('#update_font_size').click(() => {
	sendMessageToContentScript({cmd:'update_font_size', size: 50}, function(response){});
	//sendMessageToContentScript({cmd:'get_h1_content'}, function(response){console.log(JSON.stringify(response))});
});
